...
def cnn_model(
    ...
    for l in range(n_layers):
        encoder.add(Conv2D(10, (3,3), padding='same', activity_regularizer=kernel_regularizer))
...
def mlp_model(
    ...
    for l in range(n_layers):
        encoder.add(Dense(40, activity_regularizer=regularizers.l2(weight_decay)))
        encoder.add(custom_Activation(activation_name))
...
def attention_text(
    ...
    if "_approx" in activation_name:
        model.add(modular_Attention(30, activation_name='sigmoid_approx', kernel_regularizer=kernel_regularizer))
    else:
        model.add(modular_Attention(30, activation_name='sigmoid', kernel_regularizer=kernel_regularizer))
    
    model.add(Flatten())
    
    for l in range(n_layers):
        model.add(Dense(40, activity_regularizer=kernel_regularizer))
